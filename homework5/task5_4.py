...
def scrabble(word):
    ...
    my_dict = dict.fromkeys(['а', 'в', 'е', 'ё', 'и', 'н', 'о', 'р', 'с', 'т'], 1)
    my_dict.update(dict.fromkeys(['д', 'к', 'л', 'м', 'п', 'у'], 2))
    my_dict.update(dict.fromkeys(['б', 'г', 'ь', 'я'], 3))
    my_dict.update(dict.fromkeys(['й', 'ы'], 4))
    my_dict.update(dict.fromkeys(['ж', 'з', 'х', 'ц', 'ч'], 5))
    my_dict.update(dict.fromkeys(['ф', 'ш', 'э', 'ю'], 8))
    my_dict['щ'] = 10
    my_dict['ъ'] = 15
    points = 0
    for k in word:
        points = points + my_dict[k]
    return points
