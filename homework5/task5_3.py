...
def everything_for_your_cat(cats_data):
    ...
    for k in range(len(cats_data)):
        cats_data[k] = (cats_data[k][2] + ' ' + cats_data[k][3], cats_data[k][0] + ', ' + str(cats_data[k][1]))
    my_dict = {}
    for k in range(len(cats_data)):
        if cats_data[k][0] not in my_dict.keys():
            my_dict[cats_data[k][0]] = cats_data[k][1]
        else:
            my_dict[cats_data[k][0]] = my_dict[cats_data[k][0]] + '; ' + cats_data[k][1]
    # for k in range(len(our_str)):
    #    new_str = new_str + str(our_str[k]) + '_' + str(our_str[:k + 1].count(our_str[k]))
    our_str = ''
    for k in my_dict.keys():
        our_str = our_str + k + ': ' + my_dict.get(k) + '\n'
    return our_str
